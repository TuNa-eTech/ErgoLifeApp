...
Future<void> setupServiceLocator() async {
  ...
  sl.registerLazySingleton<Talker>(() => TalkerConfig.talker);
  ...
  sl.registerSingletonAsync<AuthService>(() async {
    ...
    if (AppConfig.googleClientId.isNotEmpty) {
      ...
      AppLogger.warning(
        'Google Client ID is empty, skipping Google Sign-In initialization',
        'ServiceLocator',
      );
    }
    return authService;
  ...
  sl.registerFactory<OnboardingBloc>(
    () => OnboardingBloc(userRepository: sl(), houseRepository: sl()),
  );

  // RewardsBloc
  sl.registerFactory<RewardsBloc>(
    () => RewardsBloc(rewardRepository: sl(), userRepository: sl()),
  );
  ...
  // DailyGoalBloc - factory for fresh instances
  sl.registerFactory<DailyGoalBloc>(() => DailyGoalBloc(sl()));

  // AchievementBloc - factory for fresh instances
  sl.registerFactory<AchievementBloc>(() => AchievementBloc(sl()));

  // StatsBloc - factory for fresh instances
